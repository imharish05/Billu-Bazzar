...
class Title {
  constructor({ gl, plane, renderer, text, textColor = '#C58837', font = '30px sans-serif' }) {
    autoBind(this);
...
class Media {
  ...
  onResize({ screen, viewport } = {}) {
    ...
    // Responsive card dimensions based on viewport
    const heightFraction = isMobile ? 0.55 : isTablet ? 0.50 : 0.52;
    this.plane.scale.y = this.viewport.height * heightFraction;
    this.plane.scale.x = this.plane.scale.y * (isMobile ? 0.68 : 0.72);
    this.plane.program.uniforms.uPlaneSizes.value = [this.plane.scale.x, this.plane.scale.y];
    this.padding = isMobile ? 0.6 : 1.0;
...
class App {
  constructor(container, { items, bend, textColor = '#ffffff', borderRadius = 0, font = 'bold 30px Figtree', scrollSpeed = 2, scrollEase = 0.05, onChangeActiveIndex } = {}) {
  ...
  onTouchMove(e) {
    ...
    if (Math.abs(dx) < Math.abs(dy)) { this.isDown = false; return; }
    e.preventDefault();
    const delta = dx * 0.04;
    this.scroll.target = this.scroll.position + delta * this.scrollSpeed;
  }
  ...
  update() {
    ...
    if (this.medias && this.medias[0]) {
      const width = this.medias[0].width;
      const activeIndex = Math.round(this.scroll.current / width);
      let normalizedIndex = ((activeIndex % this.itemsLength) + this.itemsLength) % this.itemsLength;
      if (normalizedIndex !== this.lastActiveIndex) {
        this.lastActiveIndex = normalizedIndex;
        if (typeof this.onChangeActiveIndex === 'function') this.onChangeActiveIndex(normalizedIndex);
      }
    }
  ...
  addEventListeners() {
    ...
    // All interaction events scoped to the container element
    ...
    this.container.addEventListener('mouseleave', this.boundMouseUp); // release drag if cursor leaves
  ...
  destroy() {
    ...
    this.container.removeEventListener('mouseleave', this.boundMouseUp);
